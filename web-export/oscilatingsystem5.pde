...
void draw() {
  ...
  if(frameCount < 100){
      //text("Hit 'd' to toggle the debugging lines." width/2, height-20);
  }
...
class Vehicle {
  ...
  void update(float osc) {
    ...
    println("min"+ minLI);
    int listItem = (int) map(velocity.mag(), 0, 10, 3, aMaj.length);

    float note = aMaj[listItem];
